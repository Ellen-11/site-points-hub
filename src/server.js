...
const localToday = () => new Date().toLocaleDateString('sv-SE', { timeZone: timezone });
...
app.get('/api/dashboard', auth, (_req, res) => {
  ...
  res.json({
    checkinToday: today,
    accounts: db.accounts.map(({ credential, refreshCookie, pricingCookie, apiKey, models, ...x }) => ({
      ...x, hasCredential: Boolean(credential), hasRefreshCookie: Boolean(refreshCookie), hasPricingCookie: Boolean(pricingCookie), hasApiKey: Boolean(apiKey), modelCount: models?.length || 0,
      checkinDate: x.checkinDate || '', checkinState: x.checkinDate === today ? 'checked' : 'unchecked'
    })),
    tags, pollTags: db.pollTags || [], runs: db.runs.slice(0, 30)
  });
});
app.get('/api/logs', auth, (req, res) => {
  ...
  const allowedActions = new Set(['', 'gateway', 'poll', 'checkin']); const allowedStatuses = new Set(['', 'ok', 'error']);
...
app.post('/api/accounts/:id/checkin-record', auth, (req, res) => {
  ...
  account.checkinDate = checked ? localToday() : '';
  db.runs.unshift({ id: crypto.randomUUID(), accountId: account.id, action: 'checkin', status: 'ok', message: checked ? `手动记录：${account.checkinDate} 已签到` : '手动记录：未签到', startedAt: new Date().toISOString() });
  db.runs = db.runs.slice(0, 5000);
  writeStore(db);
  res.json({ ok: true, checkinDate: account.checkinDate, checkinState: checked ? 'checked' : 'unchecked' });
});
...
app.post('/api/accounts/:id/:action', auth, async (req, res) => {
