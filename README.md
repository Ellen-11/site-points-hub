...
一个自托管的站点账户看板：统一查看余额/积分、按间隔轮询，并保留最近运行记录。签到不由程序自动执行，而是由人工在界面上手动记录"已签到 / 未签到"，并可一键筛选出今天还未签到的站点。
...
同一站点的手动刷新、批量轮询和定时任务会进入独立串行队列，避免并发请求重复使用一次性 refresh Cookie，或由旧任务覆盖轮换后的新 Cookie。
...
轮询由应用进程内的定时器完成，不依赖 Zeabur Cron。为避免数据文件并发写入冲突，建议只运行一个副本。应用启动后的首次轮询会在一个间隔后发生，也可在界面立即手动刷新。
...
左侧“运行日志”独立展示最多 500 条匹配记录，可按网关/轮询/签到记录、执行结果和站点筛选。
