...
降价提醒会扫描全部启用站点，并分别为按次价格和按量输入/输出价格建立基准。自动扫描只使用现有接口凭据在后台请求，遇到 401 会记录为失败，不会启动浏览器登录或切换可见页面；站点卡片上的手动刷新仍可触发浏览器续期。页面可按站点、计费方式、比较规则和模型名称筛选；首次扫描只建立基准，后续检测到更低价时才生成提醒。
